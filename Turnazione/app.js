document.addEventListener('DOMContentLoaded', () => {
    ...
    // IMPLEMENTAZIONE 1: Applicazione Filtro Visivo Fase 3 (Sbiadimento celle altri medici)
    function applyPhase3Filter() {
      ...
      shiftBoxes.forEach(box => {
        const badges = box.querySelectorAll('.cal-unavail-badge');

        if (!selectedId) {
          box.classList.remove('is-dimmed');
          badges.forEach(b => b.classList.remove('is-dimmed', 'is-highlighted'));
          return;
        }

        const hasSelectedDoc = Array.from(badges).some(b => b.dataset.docId === selectedId);

        if (hasSelectedDoc) {
          // Il medico selezionato è indisponibile qui: risalta la cella e il suo badge
          box.classList.remove('is-dimmed');
          badges.forEach(b => {
          ...
          // Il medico NON è indisponibile su questo turno: sbiadisci l'intera cella
        ...
        dayShifts.forEach(shift => {
          ...
          if (unavailDocs.length > 0) {
            unavailContent = unavailDocs.map(doc => 
              `<span class="badge badge-danger cal-unavail-badge" data-doc-id="${doc.id}" title="Indisponibile: ${doc.cognome} ${doc.nome}">${doc.cognome} ${doc.nome[0]}.</span>`
            ).join('');
          } else {
            unavailContent = `<span style="color: var(--success); font-size: 0.62rem;">✓ Disp.</span>`;
          }
